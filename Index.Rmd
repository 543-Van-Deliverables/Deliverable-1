...
```
```{r}
#How do I just get the threshold number to show as 56?
...
b1_upper_line=b1_line + annotate(geom = 'text',label=txtUpper,y = lowerT+5,x=0.2,angle=90)

b1_upper_line_out=b1_upper_line + annotate(geom = 'text',label=txtOut,y = lowerT+60,x=0.1,angle=0)
b1_upper_line_out
```
```{r}
...
library(ggplot2)
base= ggplot(CRF)  
h1= base + geom_histogram(aes(x = LifeExpectancy),
                          binwidth = WIDTH,
                          fill='darkgreen') 
h1=h1 + labs(y="count")
h1
...
h1+ geom_vline(xintercept = mn,color='green') +annotate(geom = 'text',color='green',label=annMean,y = 75,x=mn+5,angle=90) + annotate(geom = 'text', color='darkblue',label=annSk,y = 05,x=lowerT+35,angle=0)
```
